//current version WORKING VERSION UARRAY SKELETON, UARRAY UNMAPPED, UARRAY SEGEMENTS, STATIC INLINE OPERATIONS
 ...
 * Returns: nothing
*/
static inline void loadval(uint32_t *reg, uint32_t value){
    *reg = value;
}
 ...
 * Returns: nothing
*/
static inline void input(uint32_t *reg){
    int c = fgetc(stdin);
    if(c <= 255 && c >=0)
        *reg = (uint32_t)c;
    if(c == EOF)
 ...
 * Returns: nothing
*/
static inline void map_segment(uint32_t *b, uint32_t *c, struct Skeleton *skeleton){
 ...
 * Returns: nothing
*/
static inline void segmented_load(uint32_t *a, uint32_t *b, uint32_t *c, UArray_T skeletonSeq){
...
int main(int argc, char const *argv[]) {
   ...
   //  printf("skeleton sequence 0 is of size %d\n", (int)size/4); 
  //   execute(skeleton);
  ...
  //   UArray_T skeletonSeq = skeleton->skeleton_seq;
    ...
    //    printf("seg zero is %d and program counter is %d\n", (int)segZero, (int)program_counter);
        ...
        switch(currOp)
        {
            ...
            case 6:
              //  printf("bitwise nand\n");
                ...
                break;
            case 7:
               // printf("halt and program counter is %d\n", (int)program_counter);
                halt_check = halt();
                break;
            case 8:
...
void loadAndStore(UArray_T skeletonSeq, FILE *fp){
    ...
    //  printf("segment zero at index %d gets instruction %d\n", counter, (int)instruction);
      counter++;
